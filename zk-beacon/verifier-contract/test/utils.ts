...
export interface NextSyncCommittee {
    aggregate_pubkey: string;
    pubkeys: string[];
}

export function loadNextSyncCommittee(): NextSyncCommittee {
    const dataPath = path.join(projectRoot(), `${projectRoot()}/../data/lcupdate.json`);
 ...
 * 총 24624 bytes
 */
export function nextSyncCommitteeToBytes(nextSc: NextSyncCommittee): Uint8Array {
    const pubkeys = nextSc.pubkeys;
    const aggregatePubkey = nextSc.aggregate_pubkey;
